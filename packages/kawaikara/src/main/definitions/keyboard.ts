...
export const ModifierList = new Set<string>([
    'control',
    'alt',
    'meta',
    'shift',
...
export const ModifierAliasMap: Map<string, string> = new Map([
    ['ctrl', 'lctrl'],
    ['control', 'lctrl'],
    ['alt', 'lalt'],
    ['option', 'lalt'],
...
Object.freeze(ModifierAliasMap);
...
export const kawai_modifier_keys: string[] = Array.from(
    ReverseModifierKeyMap.keys(),
);
...
export function normalizeKawaiKeyLiteral(key: string) {
    const normalizedKey = key.toLowerCase();
    return (
        ModifierAliasMap.get(normalizedKey) ??
        ModifierKeyMap.get(normalizedKey) ??
        normalizedKey
    );
}

export function convertKawaiKeyCode(event: KawaiKeyEvent) {
    if (ModifierList.has(event.key.toLowerCase())) {
        // if modifier key. then convert it to kawai universal key.
