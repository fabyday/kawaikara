...
import { closeUpdateView, getUpdateState } from './update_view';
...
export function connectMainProcessHandler() {
    ...
    ipcMain.handle(KAWAI_API_LITERAL.update.status, () => {
        return getUpdateState();
    });

    ipcMain.handle(KAWAI_API_LITERAL.update.start_download, () => {
        startUpdateDownload();
        return getUpdateState();
    });

    ipcMain.handle(KAWAI_API_LITERAL.update.cancel, () => {
        cancelUpdate();
        return getUpdateState();
    });

    ipcMain.handle(KAWAI_API_LITERAL.update.install, () => {
        installUpdate();
        return getUpdateState();
    });

    ipcMain.on(KAWAI_API_LITERAL.update.close, () => {
        closeUpdateView();
    });
}
