...
observer.observe(document.body, { childList: true, subtree: true });
        ...
        (this.event2_ = async (
            ...
            if (tag === 'youtube') {
                ...
                try {
                    await fs.access(path.resolve(project_root, 'download'));
                } catch {
                    await fs.mkdir(path.resolve(project_root, 'download'), {
                        recursive: true,
                    });
                }
                ...
                //                 ? cookie.domain
                //                 : `.${cookie.domain}`;
                //             const subdomain = cookie.domain!.startsWith('.')
                //                 ? 'TRUE'
                //                 : 'FALSE';
                ...
                // const cookieString = await getCookiesString('.youtube.com');
