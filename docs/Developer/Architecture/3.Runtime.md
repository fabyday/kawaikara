...
1. Mutes the outgoing remote document, retaining its view until native replacement
   rather than presenting an interstitial loading screen. Prior failure copy is
   cleared when a retry begins.
2. Revokes outgoing page hooks/context and browser-policy authority, then captures
   its Provider/Plugin instances for background asynchronous teardown.
3. Exits PiP before transferring ownership and settles any in-progress login
   credential writes. External-browser process/profile cleanup need not finish.
4. Closes popups and the old `WebContentsView` without waiting for renderer JS.
5. Creates a new view with the selected Electron Session and loads the next
   Provider. Cookies/cache are preserved; no speculative destination is loaded.
6. Presents the new view at main-frame `dom-ready`, even while images/iframes
   still delay its full `loadURL` completion. Before the new DOM exists, only the
   themed backing color remains; this path does not impose another loading gate.
   The locale-backed failure surface appears only if activation fails.

Retired context operations cannot navigate or change the successor's internal
view, identity, cookies, external login, or page hooks. Teardown uses captured
instances rather than `currentProvider`/`currentPlugins` after an await. Provider
activations remain serialized; ordinary old async teardown is outside that
queue, while shutdown, Bundle removal, and storage resets drain retired work.
`pnpm transitions:test:handoff` exercises the real native handoff using a fresh
profile, generated decoded video, loopback pages, and deliberately held cleanup.
...
Providers may opt into `requiredCookies` and `strictCookieSynchronization`.
Required name/domain credentials are checked before any origin/cookie reset;
strict import rejects failed writes, missing/mismatched stored values, or conflicting
identities rather than reporting completion. Netflix opts in and waits for a stable
cookie jar before clearing only Netflix origins. Other Providers retain the legacy
warning-only partial-import policy unless explicitly opted in. Cookie values are
...
Chrome and then Edge. If neither launches, login fails rather than silently
using an uncontrolled browser. A Provider's ordinary
...
The public Site API `AbstractUrlProvider` performs:
...
The base Provider keeps HTTP(S) `window.open()` requests in a sandboxed,
Session-sharing popup so unknown OAuth providers still work. Override the
policy when a Provider can safely classify ordinary links and unwanted windows.
...
```mermaid
flowchart LR
  Renderer["Renderer view"] --> API["Typed window API<br/>window.kawaikara or window.kawaikaraVideo"]
  API --> Preload["contextBridge preload"]
  Preload --> IPC["ipcRenderer.invoke<br/>or event subscription"]
  IPC --> IpcManager["IpcManager"]
  IpcManager --> Managers["SiteManager / WindowManager<br/>PreferenceManager / another manager"]
```
