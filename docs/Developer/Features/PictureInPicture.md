...
`PictureInPictureManager.ts` still contains the old browser-native/Game
experiments for reference, but it is not the normal runtime path.
...
- `metadata.pictureInPicture.enabled === false` removes the shared action and
  rejects entry. The capability defaults to enabled when omitted.
...
`pictureInPicture.suppressPageControls: false` is an explicit escape hatch for
a Provider that deliberately implements a compatible page-owned lifecycle.
...
- `src/Main/Manager/UnifiedPictureInPictureManager.ts`: Provider video
  discovery, transformation, PiP window, transfer, hover, placement, and macOS
  presentation.
