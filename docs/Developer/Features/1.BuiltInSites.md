...
All built-in Providers advertise Korean, English, and Japanese locale support through their containing Bundle and Provider defaults.
...
- Waits for the external cookie jar to remain stable for 750 ms, then requires
  both `NetflixId` and `SecureNetflixId` before changing the embedded Session.
  Missing, expired, or partitioned-only credentials leave existing site data
  untouched. These are the pair described by Netflix's
  [cookie user-authentication documentation](https://github.com/Netflix/msl/wiki/Netflix-ID-Cookies-User-Authentication).
- Clears stale cache, storage, service workers, and cookies only for Netflix
  origins before importing the completed external login, then restores the
  public root URL. Unrelated cookies in a user-assigned shared profile are not
  erased. This avoids merging incompatible device/session state.
- Uses HTTPS domain-scoped import, while retaining mandatory host-only scope
  for `__Host-` cookies. Cookie write/verification failures reject login instead
  of being reported as successful; conflicting stored identities also fail.
...
Run `pnpm netflix:test:unit` for login/packaging regressions and
`pnpm netflix:test:dom` for the isolated real-Chromium cookie-storage probe.
The latter uses fake cookies and no real Netflix account or DRM playback;
Windows installed-package playback still needs a real-device check.
...
Remote integrations extend the public Site API `AbstractUrlProvider`. It
registers both DOM-ready and did-finish-load hooks before navigation, tolerates
only Electron's expected `ERR_ABORTED` SPA handoff, and runs `afterLoad()` once
more when the final load resolves.

## Maintenance checklist

- Keep one Provider per directory with `Provider.ts` and `manifest.json`, and export it from `Providers/Index.ts`.
- Put new permissions, default shortcuts, addresses, profile defaults, and
  feature switches in the Provider manifest. Put settings and DOM-dependent
  policy in `@provider()`, with translated copy in the Provider's `locale.json`.
- Mark DRM integrations accurately.
- Use a Bundle profile only when shared authentication is intentional and tested.
- Declare browser identity workarounds in the manifest and scope them to exact domains.
