...
- An edge-to-edge Explorer-style local library with drive, standard-folder, absolute
  path, and bounded recursive video search entry points.
- The app-owned native Video host has square corners so its content joins the
  viewer's title bar instead of introducing a second rounded top edge. The
  library surface has no outer padding, border, rounded card, or drop shadow;
  header/content padding and all playback/PiP backend behavior are unchanged.
- Physical mouse Back/Forward buttons navigate successful directory visits,
  including Computer/Home, rather than the `video.html` document history.
  Windows/Linux native browser app commands are routed by Main; macOS side
  buttons use capture-phase mouse events. Only the mounted folder browser
  subscribes, and listeners are removed during playback. Native requests are
  suppressed while another Provider, app overlay, or Video PiP is active.
  Failed folders do not advance history; new visits discard the forward branch.
  Rapid side-button presses are processed in sequence after each successful
  directory load, rather than repeatedly targeting an uncommitted cursor.
  Out-of-order directory and search responses cannot replace a newer visit.
...
The restricted viewer preload adds `Download with Kawaikara` to the YouTube player context menu and forwards the current HTTPS URL to Main.
...
Kawaikara validates only a bounded, credential-free HTTPS transport value. It does not decide whether the selected downloader actually supports that service or route. Source parsing and the user-facing unsupported-source failure belong to the external application.

## Download capability scope

`@kawaikara/site-api` defines one versioned `media.download` request/result contract shared by every downloader. Apps do not receive a capability name per backend. Kawaikara forwards the resource and requested features; the external application returns either acceptance or a structured failure such as `unsupported-source`, `permission-denied`, or `unsupported-version`.

The companion project ships `kawaikara-companion.json` to advertise video, time-range, quality-selection, format-selection, and its intended public scope. That declaration helps discovery and presentation, but YT Downloader remains the authoritative runtime validator. Its renderer currently reports an invalid URL instead of asking Kawaikara to maintain a duplicate yt-dlp/site policy.

Persisted companion registration and user grants remain separate follow-up work. The current launch creates a request-scoped bidirectional channel: Kawaikara starts a loopback-only HTTP callback, adds a random request ID and 256-bit bearer token to the deep link, and YT Downloader reports source acceptance or rejection, queueing, progress, completion, and failure. Callback payloads are bounded and authenticated, and failures stay isolated from both apps.

YT Downloader remains authoritative for source support. Kawaikara validates only the safe HTTPS transport envelope, so unsupported sources still reach the external app and return a structured `source.rejected` event. Kawaikara writes each callback phase to the `ExternalDownloaderManager` log source without taking ownership of the download worker.
...
For launches initiated by Kawaikara, YT Downloader associates the request ID with the queue item it creates. Queue snapshots are reduced to five-percent progress buckets before callbacks, preventing noisy per-frame logging while preserving meaningful progress, success, and failure history in Kawaikara's logger.
