...
Companion registration, persisted user grants, and authenticated local RPC remain separate follow-up work. RPC is also required for YT Downloader to return its structured result to Kawaikara; the current deep link is one-way, so validation failures are displayed by YT Downloader itself.
