...
Both Provider and internal Video PiP use the app-owned
`trackPictureInPictureVisibility` subscription. Native screen-space cursor
coordinates and current window bounds are sampled every 80 ms; controls hide
when the pointer leaves or the host is hidden/minimized. Hover remains usable
without activating the window. DOM `mouseleave`, sticky CSS `:hover`, keyboard
focus, and ordinary Video idle timers cannot force PiP controls visible.
Disposing the subscription clears both polling and the consumer's visible state.
Provider overlays use attribute presence (`data-controls-visible`) because
`toggleAttribute` creates an empty attribute, not the string `"true"`.

Run `pnpm transitions:test:unit` and `pnpm transitions:test:dom` for shared
visibility, repeated pointer entry/exit, and forced sticky-hover regression
coverage. The DOM probe uses the actual Video React component, generated video,
and isolated Chromium; `--minified` also verifies serialized page injections.
