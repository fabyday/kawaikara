...
Preferences are stored at `UserRoot/KawaiData/preferences.json`. Electron's own profiles, sessions, and caches use `UserRoot/Electron`; Kawaikara-owned configuration, video history, and diagnostic logs use `UserRoot/KawaiData`. Installed builds use channel-specific OS AppData roots named `Kawaikara`, `Kawaikara Staging`, and `Kawaikara Nightly`; a packaged development build uses `Kawaikara Dev`. Direct `pnpm dev` and `pnpm start` runs instead use `<repository>/tmp/kawaikara Dev`, keeping temporary testing completely outside every installed profile. A first run with this layout copies known Stable legacy preference and video-library files when their new destinations do not exist. Nightly and Staging also move the former duplicated channel directory to the canonical channel root when that canonical root does not yet exist. Main validates every field when loading and updating; invalid fields fall back to defaults or are omitted. Read methods return cloned arrays and objects so renderer drafts cannot mutate manager state accidentally.
